...
function ReservationCountdown({ reservedUntil }: { reservedUntil: string }) {
  ...
  return remaining > 0 ? (
    <span className="flex items-center gap-1.5">
      <Clock3 className="h-3.5 w-3.5" aria-hidden="true" /> Réservation expire dans {hours}h {minutes}m {seconds}s
    </span>
  ) : (
    ...
    <section id="properties" className="border-b border-white/5 bg-brand-dark py-20 text-white md:py-28">
      <div className="mx-auto max-w-7xl px-6 md:px-8">
        <div className="mb-12 flex flex-col justify-between gap-6 md:mb-16 md:flex-row md:items-end">
          <div>
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-gold-400 md:text-sm">Splendeur architecturale</span>
            <h2 className="mt-3 font-display text-3xl font-bold leading-tight tracking-tight sm:text-4xl md:text-5xl">Biens prestigieux en vedette</h2>
          </div>
          <button
        ...
        {filteredProperties.length === 0 ? (
          <div className="rounded-xl border border-dashed border-white/15 py-16 text-center text-gray-400">Aucun bien ne correspond à vos critères.</div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="relative h-64 w-full shrink-0 overflow-hidden md:h-72">
                    ...
                    <div className={`absolute left-4 top-4 rounded px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-white ${status.className}`}>{status.label}</div>
                    <div className="absolute right-4 top-4 rounded border border-white/10 bg-brand-dark/90 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-gold-400 backdrop-blur-md">{property.priceLabel}</div>
                  ...
                  <div className="flex flex-grow flex-col justify-between p-6">
                    <div>
                      ...
                      <h3 className="mb-2 font-display text-xl font-bold tracking-tight text-white transition-colors duration-300 group-hover:text-gold-300">{property.title}</h3>
                      ...
                      <div className="mb-6 space-y-4">
                        ...
                        <LocationMap location={property.location} latitude={property.coordinates?.latitude} longitude={property.coordinates?.longitude} />
                      </div>
                    </div>
                    <div className="flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row">
                      ...
                      >
                        <span>Demander les détails</span>
